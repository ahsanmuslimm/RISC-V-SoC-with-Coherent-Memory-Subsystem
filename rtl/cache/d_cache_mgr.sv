...
module d_cache_mgr #(
  ...
  logic [31:0] fill_data_q;         // Data from read, for merge
  ...
  logic should_fill_line;
  logic should_merge_data;
  ...
  always_comb begin
    ...
    case (state)
      ...
      CHECK: begin
        // Immediate combinational dispatch (can go to HIT_READ, HIT_WRITE, or MISS_READ same cycle)
      ...
      HIT_WRITE: begin
        // Store hit: merge bytes, write-through to SRAM
      ...
      MISS_READ: begin
        // Load/store miss: start AXI read (allocate on miss)
        bus_req          = 1'b1;
        miss_event       = 1'b1;
        state_next       = AXI_AR;
      end
      ...
      AXI_R: begin
        ...
        if (m_rvalid) begin
          ...
          state_next = AXI_R;  // Hold
        end
      end

      FILL: begin
        // Fill cache line (or bypass if uncached)
        if (!uncached) begin
          if (pending_store) begin
            cache_wr_data = merge_bytes(m_rdata, wdata_q, wmask_q);
            cache_wr_state = 2'b10;  // M (store miss)
          end else begin
            cache_wr_data = m_rdata;
      ...
      AXI_B: begin
        ...
        if (m_bvalid) begin
          ...
          state_next = AXI_B;  // Hold
        end
      end

      NOTIFY_COH: begin
        // Notify coherence controller (hold until accept, R1)
      ...
      WAIT_INVALIDATE: begin
        ...
        if (!coh_inv_valid) begin
          // Invalidation done, return to notify
          state_next = NOTIFY_COH;
        end else begin
          state_next = WAIT_INVALIDATE;  // Hold
        end
      end
  ...
  // ==================== Helper Functions ====================
  // Byte merge for store operations
